...
class FilmorateApplicationTests {
	...
	void shouldPassUserValidation() {
		...
		assertEquals(user, restTemplate.postForObject(url + "/users/add", user, User.class));
	}
	...
	void shouldPassFilmValidation() {
		Film film = new Film(0, "film", "desc", LocalDate.of(1912, 12, 28),
				Duration.ofHours(2));
		assertEquals(film, restTemplate.postForObject(url + "/films/add", film, Film.class));
	}

	@Test
	void shouldNotPassUserValidationWithEmptyLogin() {
		User user = new User(0, "email@gmail.com", "", "name", LocalDate.now().minusDays(15));
		assertNull(restTemplate.postForObject(url + "/users/add", user, User.class));
	}
	...
	void shouldNotPassFilmValidationWithEmptyName() {
		Film film = new Film(0, "", "desc", LocalDate.of(1912, 12, 28),
				Duration.ofHours(2));
		assertNull(restTemplate.postForObject(url + "/films/add", film, Film.class));
	}
